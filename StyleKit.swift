...
//  Double Pendulum
//
//  Created by Andrei Frolov on 2018-02-23.
...
public class StyleKit : NSObject {
    ...
    private struct Cache {
        ...
        static let orange: UIColor = UIColor(red: 0.898, green: 0.498, blue: 0.263, alpha: 1.000)
    }
    ...
    @objc dynamic public class func drawDoublePendulum(frame targetFrame: CGRect = CGRect(x: 0, y: 0, width: 640, height: 640), resizing: ResizingBehavior = .aspectFit, phi: CGFloat = 0, psi: CGFloat = 0, upsilon: CGFloat = 0, spread: CGFloat = 20, g: CGFloat = 0.5, braking: Bool = false) {
        //// General Declarations
        let context = UIGraphicsGetCurrentContext()!
        ...
        //// Gravity Gauge Drawing
        context.saveGState()
        context.translateBy(x: 0, y: 140)
        context.scaleBy(x: g, y: g)

        context.saveGState()
        context.setAlpha(0.5)

        let gravityGaugeRect = CGRect(x: -264, y: -264, width: 528, height: 528)
        let gravityGaugePath = UIBezierPath()
        gravityGaugePath.addArc(withCenter: CGPoint(x: gravityGaugeRect.midX, y: gravityGaugeRect.midY), radius: gravityGaugeRect.width / 2, startAngle: -alpha * CGFloat.pi/180, endAngle: -beta * CGFloat.pi/180, clockwise: true)
        gravityGaugePath.addLine(to: CGPoint(x: gravityGaugeRect.midX, y: gravityGaugeRect.midY))
        gravityGaugePath.close()

        StyleKit.mercury.setFill()
        gravityGaugePath.fill()
        ...
        if (braking) {
            //// Brake Warning
            //// Caliper 1 Drawing
            let caliper1Rect = CGRect(x: 25, y: 25, width: 590, height: 590)
            let caliper1Path = UIBezierPath()
            caliper1Path.addArc(withCenter: CGPoint(x: caliper1Rect.midX, y: caliper1Rect.midY), radius: caliper1Rect.width / 2, startAngle: 150 * CGFloat.pi/180, endAngle: -150 * CGFloat.pi/180, clockwise: true)

            StyleKit.orange.setStroke()
            caliper1Path.lineWidth = 33
            caliper1Path.stroke()


            //// Caliper 2 Drawing
            let caliper2Rect = CGRect(x: 25, y: 25, width: 590, height: 590)
            let caliper2Path = UIBezierPath()
            caliper2Path.addArc(withCenter: CGPoint(x: caliper2Rect.midX, y: caliper2Rect.midY), radius: caliper2Rect.width / 2, startAngle: -30 * CGFloat.pi/180, endAngle: 30 * CGFloat.pi/180, clockwise: true)

            StyleKit.orange.setStroke()
            caliper2Path.lineWidth = 33
            caliper2Path.stroke()


            //// Warning Drawing
            let warningRect = CGRect(x: 270, y: 162, width: 100, height: 300)
            let warningTextContent = "!"
            let warningStyle = NSMutableParagraphStyle()
            warningStyle.alignment = .center
            let warningFontAttributes = [
            ...
            context.saveGState()
            context.clip(to: warningRect)
            warningTextContent.draw(in: CGRect(x: warningRect.minX, y: warningRect.minY + (warningRect.height - warningTextHeight) / 2, width: warningRect.width, height: warningTextHeight), withAttributes: warningFontAttributes)
            context.restoreGState()
